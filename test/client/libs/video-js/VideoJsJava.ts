...
interface VideoJsJavaOptions extends VideoJsPlayerOptions {};

const MediaTechController = window.videojs.getComponent('Component');

class VideoJsJava extends MediaTechController {
    constructor(player: VideoJsPlayer, options: VideoJsJavaOptions) {
        super(player, options);
    }

    init(player: VideoJsPlayer, options: VideoJsJavaOptions, ready: any) {
        var self = this;
        MediaTechController.call(this, player, options, ready);
       
        var source = options.sources;
        ...
        var params = window.videojs.mergeOptions({
            'id': objId,
        ...
        if (player.options_.autoplay) {
            params.autoplay = player.options_.autoplay;
        }
        if (player.options_.preload) {
            params.preload = player.options_.preload;
        }
        
        // Merge default attributes with ones passed in
        var attributes = window.videojs.mergeOptions({
            'id': objId,
        ...
        if (source) {
            this.ready(function(){
                this.setAttribute('source', source?.src);
            });
        }
...
videojs.Java = VideoJsJava;
// videojs.Java = new VideoJsJava();
