...
// const junctionRoute = require('./api/routes/junction')
...
class NodeHttpServer {
	constructor(config) {
		...
		if (this.config.http.api !== false) {
			if (this.config.auth && this.config.auth.api) {
				// app.use('/api/auth', protectedRoute(context));
	...
	run() {
		this.httpServer.listen(this.port, () => {
			...
			Logger.log(this.port)
		});

		this.httpServer.on('connect', (e) => {
			Logger.error(`Node Media Http Server CONNECT ${e}`);
		});
